import { importCompanies } from "./app/models/company.server";

// Mock Admin API
const mockAdmin = {
    graphql: async (query: string, variables: any) => {
        console.log(`GraphQL Query: ${query.split('(')[0].trim()}`);
...
async function runTest() {
    ...
    const results = await importCompanies(testData as any, mockAdmin, 'csv');
    console.log("✅ Test Completed. Results count:", results.length);
